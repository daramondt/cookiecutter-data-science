...
Don't ever edit your raw data, especially not manually, and especially not in Excel. Don't overwrite your raw data. Don't save multiple versions of the raw data. Treat the data (and its format) as immutable. The code you write should move the raw data through a pipeline to your final analysis. You shouldn't have to run all of the steps every time you want to make a new figure (see [Analysis is a DAG](#analysis-is-a-dag)), but anyone should be able to reproduce the final products with only the code in `{{ cookiecutter.repo_name }}` and the data in `data/raw`.
...
Since notebooks are challenging objects for source control (e.g., diffs of the `json` are often not human-readable and merging is near impossible), we recommended not collaborating directly with others on Jupyter notebooks. There are two steps we recommend for using notebooks effectively:
 ...
 2. Refactor the good parts. Don't write code to do the same task in multiple notebooks. If it's a data preprocessing task, put it in the pipeline at `{{ cookiecutter.repo_name }}/data/make_dataset.py` and load data from `data/interim`. If it's useful utility code, refactor it to `src`.
...
# OPTIONAL: always reload modules so that as you change code in {{ cookiecutter.repo_name }}, it gets loaded
%autoreload 2
...
from {{ cookiecutter.repo_name }}.data import make_dataset
```

Note that by using {{ cookiecutter.repo_name }} as both the module name and the source folder name:
  - sources are imported similar in notebooks within your project as notebooks depending on your project.
  - one can install multiple projects that are created by this cookiecutter template, within one environment

This allows your code to be installed as a module, or to be referred by source, without changing the code.
...
If you look at the stub script in `{{ cookiecutter.repo_name }}/data/make_dataset.py`, it uses a package called [python-dotenv](https://github.com/theskumar/python-dotenv) to load up all the entries in this file as environment variables so they are accessible with `os.environ.get`. Here's an example snippet adapted from the `python-dotenv` documentation:
